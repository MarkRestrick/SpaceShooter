...
//This Boolean is used to report back whether or not the ship has collided with an asteroid
bool crashed = false;
...
void cRocket::update(float deltaTime)
{
	...
	//This sets up a bounding rectangle for the rocket ship
	RECT bounder = { spritePos2D.x, spritePos2D.y, spritePos2D.x+textureWidth, spritePos2D.y+textureHeight };
	

	//This checks if the rocket has collided with a mine
	for (vector<cAsteroid*>::iterator asteroidIterator = theAsteroids.begin(); asteroidIterator != theAsteroids.end(); ++asteroidIterator)
	{
		if ((*asteroidIterator)->collidedWith((*asteroidIterator)->getBoundingRect(), bounder))
		{
	...
	{
		if (spritePos2D.x < 800) //stops the rocket moving off of the screen
		spritePos2D.x += 10;

	}
	if (m_InputMgr->isKeyDown(VK_LEFT))
	{
		if (spritePos2D.x > 80) //stops the rocket moving off of the screen
		spritePos2D.x -= 10;
	}
	if (m_InputMgr->isKeyDown(VK_UP))
	{
		if (spritePos2D.y > 40) //stops the rocket moving off of the screen
		spritePos2D.y -= 10;
	}
	if (m_InputMgr->isKeyDown(VK_DOWN))
	{
		if (spritePos2D.y < 700) //stops the rocket moving off of the screen
		spritePos2D.y += 10;
	}
	...
	if (m_InputMgr->isKeyDown(VK_SPACE))
	{
		...
		theRocketBullets[numBullets]->setSpriteTranslation(glm::vec2(100.0f, 100.0f));
		theRocketBullets[numBullets]->setTexture(theGameTextures[5]->getTexture());
		theRocketBullets[numBullets]->setTextureDimensions(theGameTextures[5]->getTWidth()/3, theGameTextures[5]->getTHeight()/3);
...
//This function is called from the main class to check if a rocket crash has occurred
bool cRocket::checkCrash()
{

	//This if statement checks if a collision has occured, if it has it resets the checking boolean for subsequent playthroughs
	if (crashed)
	{
		crashed = false;
		return true;
